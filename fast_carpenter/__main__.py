...
from .backends import KNOW_BACKENDS_NAMES, get_backend
...
def create_parser():
    ...
    parser.add_argument(
        "sequence_cfg", type=str, help="Config for how to process events"
    )
    parser.add_argument(
        "--outdir", default="output", type=str, help="Where to save the results"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--ncores", default=1, type=int, help="Number of cores to run on"
    )
    parser.add_argument(
        "--nblocks-per-dataset",
        default=-1,
    ...
    parser.add_argument(
        "--nblocks-per-sample", default=-1, type=int, help="Number of blocks per sample"
    )
    parser.add_argument(
        "--blocksize", default=1000000, type=int, help="Number of events per block"
    )
    parser.add_argument(
        "--quiet", default=False, action="store_true", help="Keep progress report quiet"
    )
    parser.add_argument(
        "--profile", default=False, action="store_true", help="Profile the code"
    )
    parser.add_argument(
        "--execution-cfg",
        "-e",
...
def main(args=None):
    ...
    sequence, seq_cfg = fast_flow.read_sequence_yaml(
        args.sequence_cfg,
    ...
    data_import_cfg = (
        args.data_import_plugin_cfg
        if args.data_import_plugin_cfg is not None
        else args.dataset_cfg
    )
    data_import_plugin = get_data_import_plugin(
        args.data_import_plugin, data_import_cfg
    )
    ...
    results, _ = backend.execute(
        sequence, datasets, args, plugins={"data_import": data_import_plugin}
    )
