...
def tree_no_mask(infile, full_event_range):
    return m_tree.MaskedTrees({'events': infile}, event_ranger=full_event_range)
...
def tree_w_mask_bool(infile, event_range):
    ...
    mask[::2] = False
    return m_tree.MaskedTrees({'events': infile}, event_ranger=event_range, mask=mask)
...
def test_arrays_as_pandas(tree_w_mask_int, infile):
    tree_w_mask_int.apply_mask(np.arange(0, len(tree_w_mask_int), 2))
    arrays = tree_w_mask_int.arrays(["Muon_Px", "Muon_Py"], outputtype=pd.DataFrame)
